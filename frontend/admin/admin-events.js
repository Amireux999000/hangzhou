...
async function loadVotesStreamsList() {
	try {
		...
		// 兼容处理：可能是数组（新后端）或对象（旧后端）
		const streams = Array.isArray(streamsResult) ? streamsResult : (streamsResult?.streams || []);
		
		if (!streams || streams.length === 0) {
			console.warn('⚠️ 无法获取流列表或列表为空');
			return;
		}
...
async function loadAIStreamsList() {
	...
	try {
		...
		if (streams && streams.length > 0) {
			...
			// 添加所有启用的直播流
			const enabledStreams = streams.filter(stream => stream.enabled !== false);
			...
			} else if (enabledStreams.length > 0) {
				// 自动选中第一个
				aiStreamSelect.value = enabledStreams[0].id;
				// 触发 change 事件
				const event = new Event('change');
				aiStreamSelect.dispatchEvent(event);
			}
