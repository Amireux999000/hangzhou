...
// 📋 说明：当前配置
// - 直接访问Java后端服务器 (http://localhost:8081)
...
async function fetchDashboardByStream(streamId) {
	...
	const result = await apiRequest(`/api/admin/dashboard?stream_id=${streamId}`, {
		method: 'GET'
	});
...
async function apiRequest(endpoint, options = {}) {
	...
	try {
		// 🔧 修复：安全地记录请求体，防止 options.body 为对象时 JSON.parse 报错
		let logBody = '';
		if (options.body) {
		...
		console.log(`📡 API 请求: ${options.method || 'GET'} ${endpoint}`, logBody);
		...
		if (data.code === 0) {
			// 如果有 data 字段，返回 data
			// 注意：有些接口可能直接返回 null 作为 data (如 broadcastViewers)，此时也应该视为成功
...
async function fetchDashboard(streamId = null) {
	...
	if (!streamId) {
		...
		// 不直接返回错误，而是尝试获取默认 Dashboard (不传参)
		// 某些后端接口可能支持不传 stream_id 返回全局概览
		try {
			console.log('⚠️ [fetchDashboard] 尝试不带参数请求 Dashboard...');
			return await apiRequest('/api/admin/dashboard', { method: 'GET' });
		} catch (e) {
			return {
				success: false,
	...
	return await apiRequest(`/api/admin/dashboard?stream_id=${streamId}`, {
		method: 'GET'
	});
}
...
async function getStreamsList() {
	try {
		const result = await apiRequest('/api/admin/streams', {
			method: 'GET'
		});
		...
		// 2. 对象包含 streams 数组 (如 {streams: [...]})
		if (result && Array.isArray(result.streams)) {
			return result.streams;
		}
		
		// 3. 对象包含 data 数组 (如 {data: [...]})
...
async function getViewersCount(streamId) {
	...
	const result = await apiRequest(`/api/admin/live/viewers?stream_id=${streamId}`, {
		method: 'GET'
	});
...
async function getAllViewersCount() {
	...
	const result = await apiRequest('/api/admin/live/viewers', {
		method: 'GET'
	});
...
async function broadcastViewersCount(streamId) {
	...
	const result = await apiRequest('/api/admin/live/broadcast-viewers', {
		method: 'POST',
...
async function getStreamDebateTopic(streamId) {
	return await apiRequest(`/api/admin/streams/${streamId}/debate`, {
		method: 'GET'
	});
}
...
async function createDebate(debateData) {
	return await apiRequest(`/api/admin/debates`, {
		method: 'POST',
		body: JSON.stringify(debateData)
	});
}
