...
async function loadStreamsForJudges() {
	try {
		const response = await fetch(`${getAPIBaseForJudges()}/api/admin/streams`);
...
async function loadJudgesDataForStream(streamId) {
	try {
		// TODO: 调用后端API获取评委数据
		// const response = await fetch(`${getAPIBaseForJudges()}/api/admin/judges?stream_id=${streamId}`);
		// const result = await response.json();
...
async function loadUsersForSelection() {
	try {
		const response = await fetch(`${getAPIBaseForJudges()}/api/admin/users`);
		const result = await response.json();
...
async function saveJudgesData() {
	...
	try {
		// TODO: 调用后端API保存数据
		// const response = await fetch(`${getAPIBaseForJudges()}/api/admin/judges`, {
		// 	method: 'POST',
