...
class ApiService {
  ...
  async request(options) {
    ...
    // 🔧 如果没有 token，生成一个 mock token (为了本地开发方便)
    if (!authToken) {
      authToken = 'mock-token-dev-' + Date.now();
      // 也可以选择保存到本地，避免每次请求都生成新的
      try {
    ...
    // 如果存在 token，添加到请求头
    if (authToken) {
      // 🔧 后端不需要 Bearer 前缀，或者后端可能对 Authorization 格式有特殊要求
      // 这里保持 Bearer 前缀，但如果后端不支持，可能需要去掉
      defaultHeaders['Authorization'] = authToken.startsWith('Bearer ') ? authToken : `Bearer ${authToken}`;
      
      // 🔧 同时添加 x-auth-token 头，兼容某些后端
    ...
    return await apiInterceptor.requestWithRetry(async (config) => {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        // 🔧 兼容 Java 后端 ApiResponse (code=0 为成功)
        // 自动为 code=0 的响应添加 success=true，适配旧的前端逻辑
  ...
  async getVotes(streamId) {
    ...
    const url = `/api/votes?stream_id=${streamId}`;
  ...
  async getAiContent(streamId = null) {
    const url = streamId 
      ? `/api/ai-content?stream_id=${streamId}`
       : '/api/ai-content';
    return await this.request({
      url,
      method: 'GET'
  ...
  async deleteComment(contentId, commentId) {
    ...
    return await this.request({
      url: `/api/comment/${commentId}?contentId=${contentId}`,
      method: 'DELETE',
      data: {
        contentId: String(contentId) // 确保是字符串
      }
  ...
  async getDebateTopic(streamId = null) {
    const url = streamId 
      ? `/api/debate-topic?stream_id=${streamId}`
       : '/api/debate-topic';
    const response = await this.request({
      url,
      method: 'GET'
  ...
  async getUserVotes(streamId) {
    ...
    const url = `/api/user-votes?stream_id=${streamId}&user_id=${userId}`;
  ...
  async getDashboard(streamId = null) {
    ...
    const url = streamId 
      ? `/api/admin/dashboard?stream_id=${streamId}`
      : '/api/admin/dashboard';
    const response = await this.request({ url, method: 'GET' });
  ...
  async getStreamsList() {
    const response = await this.request({ url: '/api/admin/streams', method: 'GET' });
    
    // 调试日志
    console.log('📦 getStreamsList 响应:', JSON.stringify(response));

    // 格式1: 标准 Java 后端格式 {code: 0, data: {streams: [...]}}
    // 注意：拦截器可能已经添加了 success: true
    if (response && (response.success || response.code === 0) && response.data && Array.isArray(response.data.streams)) {
      return response.data.streams;
    }
    
    // 格式2: {success: true, data: [...]} (直接是数组)
    if (response && (response.success || response.code === 0) && Array.isArray(response.data)) {
      return response.data;
    }
  ...
  async getVotesStatistics(streamId = null) {
    const url = streamId 
      ? `/api/admin/votes/statistics?stream_id=${streamId}`
